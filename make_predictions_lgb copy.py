...
for pert in double_train_cols:
    ...
    # Feature Engineering
    deltaA = vecA - baseline_vec
    deltaB = vecB - baseline_vec
    interaction_vec = deltaA * deltaB

    combined_features = np.concatenate([vecA, vecB, deltaA, deltaB, interaction_vec])
...
feature_names = (
    [f"vecA_{g}" for g in genes] +
    [f"vecB_{g}" for g in genes] +
    [f"deltaA_{g}" for g in genes] +
    [f"deltaB_{g}" for g in genes] +
    [f"interaction_{g}" for g in genes]
)
...
for pert in test["perturbation"].astype(str):
    ...
    x_test_sample = np.concatenate([vecA, vecB, deltaA, deltaB, interaction_vec]).reshape(1, -1)
