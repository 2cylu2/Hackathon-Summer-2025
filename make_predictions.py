...
if len(single_cols) == 0:
    # Fallback: if the dataset names differ, at least don't crash.
    print("Warning: couldn't find single-perturbation columns by '+ctrl'. "
          "Using median across ALL columns as baseline (weaker).")
    baseline_vec = expr_agg.median(axis=1).values  # shape (1000,)
else:
    # Baseline ~ typical expression; median is robust to outliers
    baseline_vec = expr_agg[single_cols].median(axis=1).values  # shape (1000,)
